...
//混合:
//在这一课里，我们在纹理的基础上加上了混合，它看起具有透明的效果，当然解释它不是那么容易，但是希望你喜欢它。
//简单的透明
...
MyGLWidget::MyGLWidget(QWidget *parent) :
	QGLWidget(parent), mLight(false), mFullScreen(false),
	mXRotate(0.0f), mYRotate(0.0f), mXSpeed(0.0f), mYSpeed(0.0f),
	mZ(-5.0f), mLightAmbient({0.5f, 0.5f, 0.5f, 1.0f}),
	mLightDiffuse({1.0f, 1.0f, 1.0f, 1.0f}),
	mLightPosition({0.0f, 0.0f, 2.0f, 1.0f}),
	mFilter(0), mBlend(false)
{
	showNormal();
	startTimer(50);
}
...
void MyGLWidget::timerEvent(QTimerEvent *event)
{
	...
	mXRotate += mXSpeed;								// X 轴旋转
	mYRotate += mYSpeed;								// Y 轴旋转
